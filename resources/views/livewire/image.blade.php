<div>
    ...
    @elseif ( !empty($ruta_imagen) )
        <img src="{{ $path.$ruta_imagen }}" class="mx-auto rounded" alt="imagen-producto" style="width:15rem; max-height: 15rem">
    @else
    ...
    <div style="position:relative;">
		<a class='btn btn-ibizza w-100 m-1'>
			Nueva imagen
			<input wire:model="image" type="file" style='position:absolute;z-index:2;top:0;left:0;filter: alpha(opacity=0);-ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=0)";opacity:0;background-color:transparent;color:transparent;' name="imagen" size="40" onchange='$("#upload-file-info").html($(this).val());'>
		</a>
    </div>
